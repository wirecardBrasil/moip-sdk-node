...
describe('Moip Orders', function () {
    ...
    it('Should successfully get an order', function (done) {
        _index2.default.order.getOne(_order2.default.id).then(function (body) {
            body.should.be.jsonSchema(_order2.default);
            done();
        }).catch(function (err) {
            return done(err);
    ...
    it('Should fail to get an order', function (done) {
        _index2.default.order.getOne('invalid-id').catch(function () {
            return done();
        });
    });

    it('Should successfully get all orders', function (done) {
        _index2.default.order.getAll().then(function (body) {
            done();
        }).catch(function (err) {
            return done(err);
        });
